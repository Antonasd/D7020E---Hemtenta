    ...
    if file_index_current < len(file_list):
        ...
        for index, obj in enumerate(outputdata):
            if obj[4] == "Exit":
                ...
                #Computes the blocking time for each task
                task_index = tasks.index(obj[1])
                for index_t, task in enumerate(tasks):

                    if task not in blocking_time:
                        blocking_time[task] = 0

                    task_prio = int(priorities[index_t])

                    #Checks if the task can be blocked by the exiting task.
                    if int(priorities[task_index]) < task_prio and obj[3] >= task_prio:
                        if blocking_time[task] < claim_time:
                            blocking_time[task] = claim_time                        
                ...
                #Computes the WCET.
                if obj[1] not in wcet:
                    wcet[obj[1]] = 0

                if tot_time > wcet[obj[1]]:
                    wcet[obj[1]] = tot_time

            else:
                print("%s" % (obj))

        calc_cpu_demand(wcet)
        calc_response_times(blocking_time, wcet)
        calc_response_times_2(blocking_time, wcet)
...
#Assignment 2
#wcet = {task: wcet}
def calc_cpu_demand(wcet):
    print("\nAssignment 2")
    util_factor = 0

    for index, task in enumerate(tasks):
        util_factor += wcet[task]/float(interarrival[index])

    print("CPU utilization: ", util_factor)

#Assignment 3
#blocking_time = {task:blocking_time}
#wcet = {task:wcet}
def calc_response_times(blocking_time, wcet):
    print("\nAssignment 3")
    for index, task in enumerate(tasks):
        
        response_time = blocking_time[task] + wcet[task]
        interarrival_time = int(interarrival[index])

        #Preemptions calculations.
        for i, task_i in enumerate(tasks):

            if int(priorities[i]) > int(priorities[index]):
                response_time += -(-interarrival_time//int(interarrival[i]))*wcet[task_i]
        
        print(task," response time: ", response_time)

#For each tasks this function extracts the data needed to perform the preemption calculations and then
#calls calc_response_time_2 to calculate the response time.
#blocking_time = {task:blocking_time}
#wcet = {task:wcet}
def calc_response_times_2(blocking_time, wcet):
    print("\nAssignment 4")
    for index_t, task in enumerate(tasks):
        preemption_data = []
        for index_p, prio in enumerate(priorities):

            if int(priorities[index_t]) < int(prio):
                preemption_data.append([int(interarrival[index_p]), wcet[tasks[index_p]]])

        base_summ = blocking_time[task] + wcet[task]
        response = calc_response_time_2(base_summ, int(interarrival[index_t]), preemption_data, 0)

        status = "Succeeded. Response time: "+str(response[1]) if response[0] else "Failed. Deadline exceeded."
        print(task,status)

#Recursivley calculates the response time of a task.
#base_summ = Ci + Bi
#preemption_data = [[interrarival time, execution time]]. The interarrival time and execution time of the
#tasks with a higher priority than the task being evaluated.   
def calc_response_time_2(base_summ, interarrival_time, preemption_data, ri):
    ri_new = base_summ

    if ri != 0:
        for obj in preemption_data:
            ri_new += -(-ri//obj[0])*obj[1]
    if ri_new > interarrival_time:
        return [False, ri_new]
    elif ri_new == ri:
        return [True, ri_new]

    return calc_response_time_2(base_summ, interarrival_time, preemption_data, ri_new)
